...
# Comprehensive Integration Test Script for Dart SDK
...
# Read auth modes from test_config.json (test_users keys + "no-auth")
read_auth_modes() {
    local config_file="${TEST_DIR}/test/config/test_config.json"

    # Check if config file exists
    if [ ! -f "$config_file" ]; then
        echo -e "${RED}ERROR: Test config file not found: $config_file${NC}" >&2
        exit 1
    fi

    # Check if jq is installed
    if ! command -v jq &> /dev/null; then
        echo -e "${RED}ERROR: jq is not installed. Required for parsing test_config.json${NC}" >&2
        exit 1
    fi
    ...
    if [ $? -ne 0 ]; then
        echo -e "${RED}ERROR: Failed to parse test_config.json${NC}" >&2
        echo -e "${YELLOW}JSON parse error: $users${NC}" >&2
        exit 1
    fi

    if [ -z "$users" ]; then
        echo -e "${RED}ERROR: No test_users found in test_config.json${NC}" >&2
        exit 1
    fi
...
update_test_config() {
    ...
    local config_file="test/config/test_config.json"
...
run_test_mode() {
    ...
    # Set environment variable for auth mode
    ...
    # Parse results - Dart test output format with ANSI codes
    # Format: 00:12 [32m+45[0m[31m -63[0m: [31mSome tests failed.[0m
    local passed=$(grep -oE '\+[0-9]+' "$test_log" 2>/dev/null | tail -1 | grep -oE '[0-9]+' || echo "0")
    local failed=$(grep -oE ' -[0-9]+' "$test_log" 2>/dev/null | tail -1 | grep -oE '[0-9]+' || echo "0")
    local skipped=$(grep -oE '~[0-9]+' "$test_log" 2>/dev/null | tail -1 | grep -oE '[0-9]+' || echo "0")

    # Alternative: look for summary line "All tests passed!" or similar
    if [ "$passed" = "0" ] && [ "$failed" = "0" ]; then
        # Try to extract from summary
        local summary=$(grep -E "tests passed|tests failed" "$test_log" 2>/dev/null | tail -1)
        if [ -n "$summary" ]; then
            passed=$(echo "$summary" | grep -oE "[0-9]+ tests? passed" | grep -oE "[0-9]+" | head -1 || echo "0")
            failed=$(echo "$summary" | grep -oE "[0-9]+ tests? failed" | grep -oE "[0-9]+" | head -1 || echo "0")
        fi
    fi
...
main() {
    log_section "Starting Comprehensive Integration Tests (Dart SDK)"
    ...
    echo "CL Server Integration Test Results (Dart SDK) - $(date)" > "$RESULTS_FILE"
    echo "=============================================" >> "$RESULTS_FILE"
    ...
    log_info "Individual test logs in: ${RUN_LOG_DIR}/"
